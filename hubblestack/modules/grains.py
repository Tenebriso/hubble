...
def setvals(grains, destructive=False, refresh_pillar=True):
    ...
    if os.path.isfile(gfn):
        with hubblestack.utils.files.fopen(gfn, "rb") as fp_:
            try:
                grains = hubblestack.utils.yaml.safe_load(fp_)
            except hubblestack.utils.yaml.YAMLError as exc:
    ...
    for key, val in new_grains.items():
        if val is None and destructive is True:
            ...
            __grains__[key] = val
    try:
        with hubblestack.utils.files.fopen(gfn, "w+", encoding="utf-8") as fp_:
            hubblestack.utils.yaml.safe_dump(grains, fp_, default_flow_style=False)
    except (IOError, OSError):
        log.error("Unable to write to grains file at %s. Check permissions.", gfn)
    fn_ = os.path.join(__opts__["cachedir"], "module_refresh")
    try:
        with hubblestack.utils.files.flopen(fn_, "w+"):
            pass
    except (IOError, OSError):
...
def filter_by(lookup_dict, grain="os_family", merge=None, default="default", base=None):
    ...
    """
    return hubblestack.utils.data.filter_by(
